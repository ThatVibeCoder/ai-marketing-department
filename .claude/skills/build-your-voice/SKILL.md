---
name: build-your-voice
description: Derive how the owner actually writes and talks from real samples, then rewrite brand/voice.md with concrete, followable rules. Use when they want their content to sound like them, say it doesn't sound like them, want to build or fix their voice, or need writing rules a stranger could follow.
---

# Build Your Voice

Derive how the owner really writes and talks from REAL samples, never from adjectives, then REWRITE `brand/voice.md` with executable rules. The test for every rule: does it change how a sentence gets written? "Friendly and authentic" fails that test. "Never more than one comma, swears occasionally, opens with the point" passes.
...
## The ending, always
Read the one-line sound back. If samples were too thin to derive real rules, write the rules you COULD derive, add "more voice samples" to business.md's Still to answer list (same mechanic as the brain dump), and note in voice.md which rules are provisional. Save to GitHub (github agent's save-work skill, `departments/it-systems/dev-team/github-agent/skills/save-work/`). From this point forward, every writing hire reads this file first, which means corrections compound instead of repeating.
